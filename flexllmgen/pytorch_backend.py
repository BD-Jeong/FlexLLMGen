...
def _get_nvme_lba_info(tensor, device, indices):
    """Get LBA information for NVMe operations."""
    lba_info = device.tensor_lba_map[tensor.name]
    start_lba = lba_info['start_lba']
    lba_offset, indexed_shape, indexed_bytes = _calculate_lba_offset_and_size(tensor, indices, device)
    actual_start_lba = start_lba + lba_offset
    return actual_start_lba, indexed_shape, indexed_bytes
...
def copy_worker_func_nvme(queue, cuda_id):
    ...
    with torch.cuda.stream(copy_stream):
        while True:
            ...
            is_nvme_read = (src.device.device_type == DeviceType.DISK and 
            ...
            if is_nvme_write:
                ...
                def _free_after(evt, ptr, device):
                    evt.synchronize()
